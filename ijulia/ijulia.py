...
from .exceptions import CodeExecutionException, ConvertationSyntaxException
...
errors = {'CodeExecutionException': CodeExecutionException, 'ConvertationSyntaxException':  ConvertationSyntaxException}
service_host, service_port = get_service_info()
session = aiohttp.ClientSession()

@router.post("/api/v1/ijulia/generate")
...
@router.get("/api/v1/ijulia/status")
...
@router.post("/api/v1/ijulia/stop")
async def stop():
    settings.status = False
